...
We will see a quick setup to configure the ``jellyfin`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``jellyfin`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``jellyfin`` formulas under ``/srv/salt/jellyfin/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/jellyfin/parameters/dns:domain/
    ln -s dns:domain /srv/salt/jellyfin/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/jellyfin/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/jellyfin/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``jellyfin`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``jellyfin`` formulas under ``/srv/salt/jellyfin/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/jellyfin/parameters/roles
...
We create a configuration for the role ``jellyfin/server`` in ``/srv/salt/jellyfin/parameters/roles/jellyfin/server.yaml``:
...
We create another configuration for the role ``jellyfin/client`` in ``/srv/salt/jellyfin/parameters/roles/jellyfin/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles jellyfin/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles jellyfin/client
...
.. code-block:: console
    ...
    mv /srv/salt/jellyfin/parameters/dns:domain/  '/srv/salt/jellyfin/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-jellyfin-as-json:
      test.nop:
        - name: {{ jellyfin | json }}
...
Here is an example based on `template-formula/jellyfin/config/file.sls`_:
    ...
    jellyfin-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='jellyfin-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ jellyfin.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``jellyfin`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ jellyfin['winner'] }}
...
.. _template-formula/jellyfin/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/jellyfin/config/file.sls
